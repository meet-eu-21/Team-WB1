...
def makecompartimentbyGaussianHMM(chrmat, N=2):
    ...
    model = hmm.GaussianHMM(n_components=N, n_iter=1000, covariance_type="diag").fit(
        chrmat)  # n_iter has to be >100 after test
    # print(model)
...
def writePDB(fnameout, value, EpiValue):
    ...
    while i <= Sh[0]:
        ...
        S = S.format('ATOM', i, 'CA', '', 'ALA', 'A', i, '', float(value[i - 1, 0]), float(value[i - 1, 1]),
                     float(value[i - 1, 2]), 1, float(EpiValue[i - 1]), 'C', '')
        # print(S)
        fout.write(S + "\n")
        i += 1
    fout.close()
